...
| `brstormhitch` / `brstormbisect` | Correlate #350's long frames with storm/network/map markers, then run clean local-render A/B windows: map fill absent, resident-but-frozen, position-only without resize, shaped wall absent, and normal restoration. |
