...
| `brstormbisect <normal\|mapoff\|mapfreeze\|mapnoresize\|mapnokeys\|walloff> [ms]` | client | Runtime A/B for #350 and #344. Each mode starts a fresh hitch capture while changing only local rendering: remove the custom map fill, freeze its live updates, suppress only its resize call, draw one never-faded keyframe as 52a7caa did (no alpha writes, one rebuild as a sweep finishes), or suppress the shaped 3D wall. `normal` restores shipping behavior |
