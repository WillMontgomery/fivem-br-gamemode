...
| **Voice modes** | `nearby`, `squad`, `off`, set under **Settings → Voice**, and **mutually exclusive rather than layered** (`BR.VoiceRouting`, `br_lib/shared/enums.lua`: two booleans per mode, never both true). `nearby` is proximity with falloff and **no** radio — a squadmate across the island is as inaudible as any stranger. `squad` is the server-assigned pma-voice **radio channel and nothing else**: no falloff, no proximity, and every non-squadmate muted. `off` transmits to nobody and hears nobody. **The default is `nearby`**, defined once in `br_lib/shared/enums.lua` (`BR.VoiceModeDefault`) and read by both `br_ui/client/settings.lua` and `br_core/client/voice.lua`. Two things about `squad` are not obvious and are why it was reported broken (#157): **it has its own push-to-talk** — pma-voice's `+radiotalk`, "Talk over Radio", default **Left Alt** — and the ordinary voice key carries nothing in that mode; and **squad mode with no squad is total silence**, which is correct and identical to a fault. Both are now said on the HUD (`ui-src/src/hud/VoiceNotice.tsx`), on the settings screen and in `/brvoice`, from one source: `BR.Voice.statusFor`. The Squad option is **shown but disabled** in solo rather than hidden — hiding it left a player whose stored mode was `squad` with no button highlighted and no explanation. |
