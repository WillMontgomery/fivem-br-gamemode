...
| Term | Meaning |
|---|---|
| **Roster** | The server's per-player table (`br_core/server/roster.lua`) — the single source of truth for who is connected, their state, squad, health, kills, placement. Positions and licenses live here too but are never broadcast. |
...
| Term | Meaning |
|---|---|
| **POI** | Point of interest: **120** places (`br_lib/config/map.lua`) with a tier that drives loot density — 75 tier 1, 28 tier 2, 13 tier 3, **4 tier 4**. Tier 4 is the four **golden** POIs (#227, 2026-09-08): `humane`, `kortz`, `raton` and `chaparral`, which spawn 35 crates on their own rarity row and are deliberately indistinguishable from any other POI on a player's map. **72 are north of the city** (`y > 500`), and **23** of those are **backcountry** — sited from the empty ground *between* the road corridors rather than from a place name, because named places are all on roads and the authored set had inherited the road network's shape. The backcountry set was 25 until 2026-08-28, when the surveyed map boundary put `graybeard` and `calafia` outside it by 51 m and 45 m; nothing else in the set moved. `tools/check_pois.lua` gates the spacing and the distance to the nearest corridor, and prints all four counts on every run. They double as storm-anchor candidates. |
...
| **Cell** | The streaming grid, 256 m squares. Clients subscribe to the 3×3 block around them and are sent only those entries — a small fraction in flight instead of the whole layout (~3,690 entries: 2,456 crates + 813 floor items + 420 roadside filler). Props are a much smaller radius again (90 m), because entries are cheap and objects are not. |
...
| **Rolled dirt** | Since 2026-09-01, every showroom car also wears a dirt level the server rolls per row per match (`BR.ShopSolve.dirt`), applied to the display car and to the car that comes out of the item alike — owner: *"can you make vehicles have random dirt levels >20% at the shop? That part should also copy over once they've spawned it."* It is a **second derivation from the same `shopSeed`**, not a second seed, and nothing about dirt travels over the wire. `SET_VEHICLE_DIRT_LEVEL` runs **0.0–15.0, not a percentage**, so ">20%" is **3.0**; the band is **(3.0, 9.0]** — 20% to 60% — with the top of the scale left free because the engine keeps adding dirt as a car is driven, and a car delivered at 15.0 could never look like it had been anywhere. **The ambulance is NOT excluded**, unlike the colour roll: that exemption exists because paint covers a livery, and dirt weathers a livery rather than replacing it. The per-row opt-out is `randomDirt = false` and no shipped row uses it. |
...
| **Report reward** | 100 Volts to the reporter and every corroborator when their case resolves with an action taken (250 in #168, 125 from the 2026-08-20 halving that swept it up, 100 by the owner's own decision on 2026-09-02 — see #256). The debt is queued durably on the game's own table because the verdict arrives days and several deploys later; the credit and the receipt are one conditional write, so it cannot be paid twice. See [progression.md](progression.md). |
