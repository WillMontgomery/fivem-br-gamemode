...
-- BR-PATCH 3b (fivem-royale): the Left Alt "Talk over Radio" KEY MAPPING is
-- gone. THE +radiotalk AND -radiotalk COMMANDS ABOVE ARE UNTOUCHED, AND MUST
-- STAY THAT WAY -- they are this gamemode's entire squad transmit path.
--
-- br_core owns push-to-talk as `brptt` (default N, rebindable on our own
-- settings screen, br_core/client/keybinds.lua) and drives this resource with
-- ExecuteCommand('+radiotalk') / ExecuteCommand('-radiotalk') from
-- br_core/client/voice.lua. Deleting either RegisterCommand above makes squad
-- voice silent.
--
-- What went is only the RegisterKeyMapping, and with it the row in FiveM's
-- keybinding menu. Owner, verbatim: "Why is left alt used for anything? That
-- was a mistake - a default left in place." LMENU was never chosen; it is this
-- line's default, inherited by never setting voice_defaultRadio. Setting that
-- convar would only have MOVED the second binding -- it would still have been a
-- key registered outside our key layer, absent from our settings screen and
-- from BR.Keys.labelFor, and still unmoved for a player who had already
-- rebound it. So the second layer goes rather than moves.
--
-- Original line, for the next upstream bump:
