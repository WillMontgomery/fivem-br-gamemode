...
function toggleVoice(plySource, enabled, moduleType)
	...
	if enabled and (not distance or distance > 4.0) then
		-- BR-PATCH 1 (fivem-royale): was a bare, unconditional
		-- `print(volumes[moduleType])`. It put a raw number -- "0.6" -- into
		-- every player's F8 console every time anybody within earshot started
		-- talking, with no label and no way to turn it off. Same value, through
		-- this resource's own logger, which is gated on voice_debugMode.
		logger.verbose('[main] Volume override for %s (%s): %s', plySource, moduleType, volumes[moduleType])
...
function playMicClicks(clickType)
	-- BR-PATCH 2 (fivem-royale): NO BUTTON CHIRP, EVER. Owner, from the
	-- playtest: "It still makes a radio chirp sound when pressing and releasing
	-- the button... I don't hate the radio sound effect, but the button chirp
	-- needs to go."
	--
	-- THE RADIO SOUND EFFECT IS NOT THIS FUNCTION AND IS DELIBERATELY UNTOUCHED.
	-- That is the SUBMIX (SetAudioSubmixEffectRadioFx, client/init/submix.lua,
	-- applied in toggleVoice above under voice_enableSubmix). This function only
	-- ever played ui/mic_click_on.ogg and ui/mic_click_off.ogg.
	--
	-- Killed HERE rather than at the three call sites in client/module/radio.lua
	-- because one early return cannot be missed by a later upstream that adds a
	-- fourth caller. Killed unconditionally rather than by flipping the
	-- `micClicks` default because that flag is reachable from the
	-- setVoiceProperty export and from a per-machine KVP, so "off by default"
	-- is a value any resource or any stale KVP can put back.
	do return end
